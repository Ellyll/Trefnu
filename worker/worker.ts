namespace Trefnu.Worker {
    ...
    function qs<T>(A: T[], comparer, lo, hi, onswap, onfinish) {
        if (lo < hi) {
    ...
    function quicksort<T>(array: T[], comparer, onswap, onfinish) {
        qs(array, comparer, 0, array.length-1, onswap, onfinish);
        onfinish();
    }

    // Bubblesort
    function bubblesort<T>(array: T[], comparer, onswap, onfinish) {
        // Dim angen trefnu
        if (array.length <=1) {
            onfinish();
            return;
    ...
    function swap<T>(a : T[], i : number, j : number, onswap : (a: number, b: number) => void) {
        if (i >= a.length)
    ...
    // Heapsort
    function iParent(i : number) {
        return Math.floor((i-1) / 2); //where floor functions map a real number to the smallest leading integer.
    }
    function iLeftChild(i : number) { return 2*i + 1; }
    function iRightChild(i : number) { return 2*i + 2; }

    function heapify<T>(a: T[], count : number, comparer, onswap) {
    ...
    function siftDown<T>(a : T[], start : number, end : number, comparer, onswap) {
        let root = start;
        ...
        while (iLeftChild(root) <= end) { //    (While the root has at least one child)
            ...
            if (comparer(a[swp], a[child]) < 0) //  a[swp] < a[child])
                swp = child;
            //(If there is a right child and that child is greater)
            if (child+1 <= end && comparer(a[swp], a[child+1]) < 0)  //a[swp] < a[child+1])
                swp = child + 1;
            if (swp == root)
    ...
    function heapsort<T>(a : T[], comparer, onswap, onfinish) {
    ...
    function selectionsort<T>(a : T[], comparer, onswap, onfinish){
    ...
    addEventListener('message', evt => {
        if (evt.data.type === 'trefnu') {
            ...
            const cyfnewidiadau : number[][] = [];

            const arCyfnewid = (a : number, b : number) => {
